# -*- coding: utf-8 -*-
# Unified TopBottom snapshot builder (no liquidity screens, no performance)
#
# This is the merge of:
#   - _0005_TopBottom10.py          (Base, LightGBM, Ridge)
#   - _0005_TopBottom10-add.py      (LightGBMScore, RidgeScore)
#
# Inputs (unchanged):
...
#   - LightGBMScore:LightGBMScoreRanking/year=*/LightGBMscore_rank_YYYYMMDD.parquet
#   - RidgeScore:   RidgeScoreRanking/year=*/ridgescore_rank_YYYYMMDD.parquet
#
# Outputs (unchanged):
...
MODEL_SPECS = [
    ...
    # LightGBMScore model (pred_12m as score)
    {
        "name":          "LightGBMScore",
        "in_dir":        BASE / "LightGBMScoreRanking",
        "pattern":       "LightGBMscore_rank_*.parquet",
        "score_col":     "pred_12m",
        "required_cols": {"date", "permno", "pred_12m"},
        "out_dir":       BASE / "LightGBMScoreTopBottom10",
        "out_cols":      ("date", "permno", "signal"),
    },
    # RidgeScore model (pred_12m as score)
    {
        "name":          "RidgeScore",
        "in_dir":        BASE / "RidgeScoreRanking",
        "pattern":       "ridgescore_rank_*.parquet",
        "score_col":     "pred_12m",
        "required_cols": {"date", "permno", "pred_12m"},
        "out_dir":       BASE / "RidgeScoreTopBottom10",
        "out_cols":      ("date", "permno", "signal"),
    },
]

# Create output dirs (roots)
...
# Keep the historical filename prefix
FILE_PREFIX = "topbottom10"  # 10% longs + 10% shorts = 20% total
...
def make_topbottom(df: pl.DataFrame, score_col: str) -> tuple[pl.DataFrame, int, int]:
    """
    Shared top/bottom 10% selection logic.
    Returns (out_df_with_signal, top_n, bot_n).
    """
    cols = set(df.columns)
    ...
    if "date" in cols:
        exprs.append(pl.col("date"))  # keep as-is
    ...
    # ensure disjoint (mostly relevant for tiny universes)
    if bot.join(top.select("permno"), on="permno", how="inner").height > 0:
        top_permnos = top.get_column("permno")
        bot = bot.filter(~pl.col("permno").is_in(top_permnos))
        if bot.height < bot_n:
            fill = (
                bottom_sorted
                .filter(~pl.col("permno").is_in(top_permnos))
    ...
    out = pl.concat([top, bot], how="vertical_relaxed")
    return out, top_n, bot_n
...
def build_topbottom_for_model(spec: dict) -> None:
    ...
    for f in files:
        ...
        out_all, top_n, bot_n = make_topbottom(df, score_col)
        if out_all.height == 0:
        ...
        keep = [c for c in out_cols if c in set(out_all.columns) | {"signal"}]
        out = out_all.select(keep)
