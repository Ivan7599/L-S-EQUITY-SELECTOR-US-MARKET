# -*- coding: utf-8 -*-
# Multi-model Performance script — Buy&hold sleeves, snapshot-day rebal
# For each model:
#   - reads TopBottom10/year=*/topbottom10_YYYYMMDD.parquet  (permno, signal)
#   - builds cohort sleeves and aggregate daily L/S
#   - saves LS + legs returns, perf CSV, and VAMI plots
...
def run_for_model(spec: dict, mkt_df: pl.DataFrame) -> None:
    ...
    perf_df = pl.DataFrame({
        ...
        "avg_daily_ls": [mu_ls,         mu_mkt],   # legacy col name
    ...
    print(
        f"[{name}] Saved: {OUT_IMG}, {OUT_CSV}, {OUT_RET}"
    )
    print(
        f"[{name}] Saved: {OUT_LONG_IMG}, {OUT_SHORT_IMG}, {OUT_LONG_RET}, {OUT_SHORT_RET}"
    )
